# rpos — development notes

> Hardware bring-up, JTAG/QEMU debugging setup, and an AArch64 quick-reference,
> collected while starting this project. Kept for reference — some details (e.g.
> exact `make` targets) may lag the current build. For an overview of what the
> project is today, see [README.md](README.md).
...
```c
// JTAG init above
DEBUG("Waiting for debug gate to be released...\n");
int gate = 0;
while(!gate);
```
