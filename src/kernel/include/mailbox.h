...
typedef struct mailbox_tag_s{
    uint32_t id;
    uint32_t buffer_size;
    uint32_t value_length;
} mailbox_tag_t;

typedef struct mailbox_command_s{
    uint32_t size;
    void* tag;
} mailbox_command_t;

typedef struct mailbox_generic_s{
    mailbox_tag_t tag;
    uint32_t id;
    uint32_t value;
} mailbox_generic_t;

typedef struct mailbox_power_s{
    mailbox_tag_t tag;
    uint32_t id;
    uint32_t state;
} mailbox_power_t;

typedef struct mailbox_clock_s{
    mailbox_tag_t tag;
    uint32_t id;
    uint32_t rate;
} mailbox_clock_t;
...
#define REGS_MAILBOX ((struct mailbox_regs*) (PBASE + 0xB880))
